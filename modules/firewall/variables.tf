...
variable "firewall_name" {
  type        = string
  description = "Name of the Azure Firewall (must be unique within region)"

  validation {
    condition     = can(regex("^[a-zA-Z0-9][a-zA-Z0-9-]{0,78}[a-zA-Z0-9]$", var.firewall_name))
    error_message = "Firewall name must be 1-80 characters, start/end with alphanumeric, and contain only alphanumeric or hyphens"
  }
}

variable "public_ip_name" {
  type        = string
  description = "Name of the primary public IP for the firewall"
